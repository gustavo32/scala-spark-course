...
val assembler = new VectorAssembler().setInputCols(data.drop("Region").columns).setOutputCol("features")
...
val silhouette = evaluator.evaluate(model.transform(cleanedData))
