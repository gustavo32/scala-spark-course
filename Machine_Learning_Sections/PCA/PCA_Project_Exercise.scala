...
val data = spark.read.option("inferSchema", true).option("header", true).csv("Cancer_Data")
...
val cleanedData = assembler.transform(data).select("features")
...
val scaledData = scaler.fit(cleanedData).transform(cleanedData)
...
val pcaDF = pca.fit(scaledData).transform(scaledData).select("pcaFeatures")
